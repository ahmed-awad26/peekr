...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            PeekrTheme {
                PeekrApp(securityManager = securityManager)
            }
        }
    }
...
@Composable
fun PeekrApp(securityManager: SecurityManager) {
    ...
    // انتظر DataStore يجيب القيمة
    if (onboardingDone == null) return

    // Onboarding أول مرة
    if (!onboardingDone!!) {
        OnboardingScreen(onFinish = { /* ViewModel حيعمل markDone */ })
        return
    }
